...
export class KnowledgeGraphManager {
    ...
    validateRelation(relation, graph) {
    ...
    filterNewEntities(newEntities, existingEntities) {
        return newEntities.filter(e => !existingEntities.some(existing => existing.name === e.name));
    }
    filterNewRelations(newRelations, existingRelations) {
        return newRelations.filter(r => !existingRelations.some(existing => existing.from === r.from &&
            existing.to === r.to &&
            existing.relationType === r.relationType));
    }
    async addObservations(updates) {
        const graph = await this.loadGraph();
        try {
    ...
    findEntityOrThrow(entityName, graph) {
    ...
    filterNewObservations(newObs, existingObs) {
    ...
    removeDeletedEntities(entityNames, entities) {
        return entities.filter(e => !entityNames.includes(e.name));
    }
    removeRelationsWithDeletedEntities(entityNames, relations) {
        return relations.filter(r => !entityNames.includes(r.from) && !entityNames.includes(r.to));
    }
    ...
    filterRemainingRelations(deletedRelations, existingRelations) {
        return existingRelations.filter(existing => !deletedRelations.some(deleted => existing.from === deleted.from &&
    ...
    filterRelationsByEntities(entityNames, relations) {
        return relations.filter(r => entityNames.has(r.from) && entityNames.has(r.to));
    }
