...
let memoryFilePath = '';
// Check for --path or -p argument
const pathIndex = args.findIndex(arg => arg === '--path' || arg === '-p');
if (pathIndex !== -1 && args[pathIndex + 1]) {
    memoryFilePath = args[pathIndex + 1];
}
else {
    // Default to memory.json in the current directory if no path specified
    const __dirname = path.dirname(fileURLToPath(import.meta.url));
    memoryFilePath = path.join(__dirname, 'memory.json');
}
...
try {
    ...
    console.error(`Failed to create directory ${memoryDir}:`, error);
    process.exit(1);
}
...
// Initialize the manager with the specified file path
